...
-- 3bis. Résumé du portefeuille utilisateur
create or replace view public.user_portfolio_summaries as
select
  ui.user_id,
  sum(coalesce(ui.shares_bought * io.price_per_share, 0)) as total_portfolio_value,
  sum(coalesce(ui.amount_invested, 0)) as total_invested,
  sum(coalesce(ui.shares_bought * io.price_per_share, 0)) - sum(coalesce(ui.amount_invested, 0)) as total_gain,
  case when sum(coalesce(ui.amount_invested, 0)) > 0 then
    ((sum(coalesce(ui.shares_bought * io.price_per_share, 0)) - sum(coalesce(ui.amount_invested, 0))) / sum(coalesce(ui.amount_invested, 0))) * 100
  else 0 end as total_gain_pct
from public.user_investments ui
left join public.investment_offers io on ui.offer_id = io.id
where ui.status = 'actif' and ui.shares_bought > 0
group by ui.user_id;
...
-- 6bis. Automatisation des recalculs de portefeuille
create or replace function public.sync_portfolio_balances_on_price_change()
returns trigger as $$
begin
  if NEW.price_per_share is distinct from OLD.price_per_share then
    with computed AS (
      select
        ...
        (ui.shares_bought * NEW.price_per_share) as new_value,
        (ui.shares_bought * NEW.price_per_share) - coalesce(ui.current_value, ui.amount_invested) as balance_adjustment
      from public.user_investments ui
      where ui.offer_id = NEW.id
        and ui.status = 'actif'
    ...
    updated_investments AS (
      ...
      returning c.user_id, c.balance_adjustment
    ),
    user_totals AS (
      select user_id, sum(balance_adjustment) as total_adjustment
    ...
    select
      ...
      abs(ut.total_adjustment),
      'complété',
      case
        when ut.total_adjustment >= 0 then 'Plus-value suite mise à jour des cours' 
        else 'Ajustement suite baisse des cours' 
      end
    from user_totals ut
    where abs(ut.total_adjustment) >= 0.01;
  end if;
...
create trigger trg_sync_portfolio_balances_on_price_change
  ...
  for each row execute procedure public.sync_portfolio_balances_on_price_change();
