    ...
    <GlassCard padding="none" hover={false} className="overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left min-w-[480px]">
          <thead className="sticky top-0 bg-fin-card/95 backdrop-blur">
            <tr className="border-y border-white/5 text-[11px] uppercase tracking-wider text-fin-mute">
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-white/5 hover:bg-fin-hover/50 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-fin-surface border border-white/10 flex items-center justify-center text-[10px] font-bold text-fin-primary">
                          {initialsFromTitle(row.title)}
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-white">{tickerFromTitle(row.title)}</p>
                          <p className="text-[11px] text-fin-mute truncate max-w-[140px]">{row.title}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-3.5 text-sm font-medium text-white">{row.quantity}</td>
...
export function PortfolioCard({
  ...
  return (
    <GlassCard padding="sm" className="flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-2xl bg-fin-surface border border-white/10 flex items-center justify-center text-xs font-bold text-fin-primary">
          {initialsFromTitle(title)}
        </div>
        <div className="min-w-0">
          <p className="font-bold text-sm truncate">{ticker || tickerFromTitle(title)}</p>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          <p className="font-semibold">{price.toLocaleString('fr-FR')}</p>
        </div>
        <div>
          <p className="text-[10px] text-fin-mute uppercase tracking-wider">Valeur</p>
          <p className="font-semibold">{formatFcfa(value ?? shares * price)}</p>
        </div>
        <div>
